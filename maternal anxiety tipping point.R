...
dat$v01bas_livingchd_cat    <- ordered(dat$v01bas_livingchd_cat)  # ordered; MI via polr
dat$maternal_anxiety                 <- as.factor(dat$maternal_anxiety)

# Flag who was missing maternal_anxiety pre-imputation 
dat$was_missing_maternal_anxiety <- is.na(dat$maternal_anxiety)
...
meth["maternal_anxiety"] <- "logreg"

# Ordered categorical predictor has 15 missings → impute with proportional odds model
meth["v01bas_livingchd_cat"] <- "polr"

# Predictors used for maternal_anxiety imputation (MAR model; unadjusted analysis comes later)
pred["maternal_anxiety", ] <- 0
pred["maternal_anxiety", c(trt_col, "v01bas_age_2","v01bas_canread_4","v01bas_canwrite_5",
...
# Quick checks
plot(imp)
densityplot(imp, ~ maternal_anxiety)
...
form_pred <- reformulate(
  c(trt_col, "v01bas_age_2","v01bas_canread_4","v01bas_canwrite_5",
    "v01bas_livingchd_cat","v01bas_weight_14","v01bas_gestage_17"),
  response = "maternal_anxiety"
)

# Unadjusted analysis model 
form_bin_unadj <- reformulate(trt_col, response = "maternal_anxiety")

# Apply a common δ shift to imputed outcomes in NON-reference arms
adjust_imputed_binary <- function(d, delta) {
  d <- set_ref(d)
  d_obs <- d[!d$was_missing_maternal_anxiety, , drop = FALSE]
  ...
  idx <- which(d$was_missing_maternal_anxiety & d[[trt_col]] != ref_label & is.finite(p_del) & !is.na(p_del))
  if (length(idx)) d$maternal_anxiety[idx] <- rbinom(length(idx), 1, p_del[idx])
  d
}
...
run_one_delta <- function(delta) {
  ests <- purrr::map_dfr(seq_len(m), function(i) {
    ...
    d_i$was_missing_maternal_anxiety <- dat$was_missing_maternal_anxiety
...
adjust_imputed_binary_perarm <- function(d, delta_map) {
  ...
  for (arm in names(delta_map)) {
    idx <- d$was_missing_maternal_anxiety & d[[trt_col]] == arm
    if (any(idx)) p_del[idx] <- plogis(qlogis(p0[idx]) + delta_map[[arm]])
  }
  p_del <- pmin(pmax(p_del, 1e-8), 1 - 1e-8)
  idx_all <- which(d$was_missing_maternal_anxiety & d[[trt_col]] %in% names(delta_map) & !is.na(p_del))
  if (length(idx_all)) d$maternal_anxiety[idx_all] <- rbinom(length(idx_all), 1, p_del[idx_all])
  d
}
...
# ==== Tipping-point interpretability: % of missing anxious at the tip =========
# Needs in your environment: imp, dat, trt_col, ref_label, form_pred, tips_4

# Safety: ensure reference level helper exists
set_ref <- function(d) {
  d[[trt_col]] <- stats::relevel(factor(d[[trt_col]]), ref = ref_label)
  d
}

# 1) Pool MAR baseline probabilities p0 for EVERYONE across imputations
#    Fit the predictor model on the rows that were originally OBSERVED
get_p0_maternal_anxiety_from_imp <- function(imp_obj, form_pred, trt_col, ref_label, original_dat) {
  m <- imp_obj$m
  n <- nrow(mice::complete(imp_obj, 1))
  pmat <- matrix(NA_real_, nrow = n, ncol = m)
  
  obs_idx <- which(!original_dat$was_missing_maternal_anxiety)
  
  for (i in seq_len(m)) {
    d_i <- mice::complete(imp_obj, i) |> set_ref()
    # Fit on originally observed outcome rows (MAR model)
    fit_i <- suppressWarnings(glm(form_pred,
                                  data = d_i[obs_idx, , drop = FALSE],
                                  family = binomial()))
...
# 2) Summarize, among the MISSING in a given arm, what % would be anxious at δ*
summarize_missing_arm_all <- function(original_dat, p0_list, arm, delta_star) {
  d2 <- set_ref(original_dat)
  p0 <- p0_list$p0
  
  idx_miss <- d2$was_missing_maternal_anxiety & d2[[trt_col]] == arm
  M <- sum(idx_miss)
  if (M == 0) {
    return(dplyr::tibble(
      arm = arm, M_missing = 0,
...
# 3) Wrapper across the four panels, applying δ* and the correct arm(s)
calc_tip_requirements_all <- function(original_dat, tips_tbl_long) {
  d0 <- set_ref(original_dat)
  p0_list <- get_p0_maternal_anxiety_from_imp(imp, form_pred, trt_col, ref_label, original_dat = d0)
  ...
  purrr::map_dfr(seq_len(nrow(tips_tbl_long)), function(i) {
    ...
    purrr::map_dfr(arms_to_shift,
                   ~ summarize_missing_arm_all(d0, p0_list, .x, delta_star)) %>%
      dplyr::mutate(
        panel = panel,
        direction = direction,         # add direction label
        delta_star = delta_star,
        odds_multiplier = exp(delta_star)
      ) %>%
      dplyr::select(panel, direction, delta_star, odds_multiplier,
                    arm, M_missing, p0_mean, p_tip_mean, pct_needed, extra_cases)
  })
}


# ---- Run + pretty print -------------------------------------------------------
tip_requirements_anx <- calc_tip_requirements_all(dat, tips_4_long)
...
# --- Pretty summary table of tipping deltas (both directions) -----------------

tip_requirements_anx %>%
  dplyr::mutate(
    dplyr::across(
...
library(openxlsx)
write.xlsx(tip_requirements_anx, "anxiety_tipping_point_results_table.xlsx", rowNames = FALSE)
...
p4 <- ggplot(df_all, aes(x = delta, y = estimate)) +
  ...
  theme_bw() +
  labs(
    title    = "Tipping-point sensitivity to missing outcomes — EPDS-3a ≥ 6 (binary)",
...
print(p4)
ggplot2::ggsave("tp_maternal_anxiety_four_panel_faceted.png", p4, width = 11, height = 9, dpi = 300)
