...
excluded_df <- tabmev5_kb[!tabmev5_kb$midm %in% included_ids, ]
...
muller_study_sub <- muller_study %>% select(all_of(vars))
excluded_sub <- excluded_unique_mothers %>% select(all_of(vars))
...
table1 %>%
  as_flex_table() %>%
  save_as_docx(path = "Comparing included vs excluded population for children's developement.docx")


# compare the standard mean differences

library(gtsummary)

compare_clean <- compare_subset %>%
  mutate(
    v01bas_canread_4 = v01bas_canread_4 == "yes",
    v01bas_canwrite_5 = v01bas_canwrite_5 == "yes",
    v01bas_abort_7 = v01bas_abort_7 == "yes",
    v01bas_anydisease_13 = v01bas_anydisease_13 == "yes"
  )
...
class(compare_clean$v01bas_canread_4)
table(compare_clean$v01bas_canread_4)
